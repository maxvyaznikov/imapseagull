...
function runClientMockup(port, host, commands, debug, callback) {
    ...
    socket.on("error", function(err) {
        console.log("Connection trying with %s:%s failed", host, port);
    ...
    socket.on("connect", function() {
        ...
        socket.on("close", function() {
            ...
            callbackSent = true;
            if (typeof callback == "function") {
                callback(null, Buffer.concat(responses));
            }
        });
        ...
        socket.on("data", function(chunk) {
            ...
            if (command.match(/^[a-z0-9]+ STARTTLS$/i)) {
                ...
                var secureSocket = tls.connect({
                    ...
                    secureSocket.on("close", function(err) {
                        if (callbackSent) {
                            return;
                        }
                        callbackSent = true;
                        if (typeof callback == "function") {
                            callback(err, Buffer.concat(responses));
                        }
                    });
