...
IMAPConnection.prototype.indexFolders = function(callback) {
    var me = this;
    me.folders = deepExtend((me.user&&me.user.folders)||{}, this.options.folders);
    me.folderCache = {};
        ...
        Object.keys(branch).forEach(function(key) {
            ...
            me.processMailbox(curPath, curBranch, namespace, function() {
                if (namespace != 'INBOX' && curBranch.folders && Object.keys(curBranch.folders).length) {
                    walk_tree(curPath, separator, curBranch.folders, namespace, alter_walk_tree);
                }
                walk_tree_count--;
                if (walk_tree_count === 0 && alter_walk_tree) {
                    alter_walk_tree(callback);
                }
            }.bind(this));
        });
    }

    function alter_walk_tree(callback) {
...
IMAPConnection.prototype.send = function(data) {
    ...
    var compiled = this.server.options.imapHandler.compiler(data);
...
IMAPConnection.prototype.scheduleCommand = function(data) {
    ...
    try {
        parsed = this.server.options.imapHandler.parser(data, {literalPlus: this.server.literalPlus});
    } catch (e) {
        console.error(e.message, e.stack);
        this.send({
            tag: '*',
